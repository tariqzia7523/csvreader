...
public class CsvFileDataSourceImpl implements TableDataSource<String, String, String, String> {
    ...
    private InputStreamReader getInputStreamReader() throws Exception {
        try{
    ...
    void init() {
        ...
        try {
            ...
            int status = FileUtils.copy(originalFile, tempFile);
            if(status == 1){
...
//            else
//                Toast.makeText(mContext,mContext.getString(R.string.can_not_read_read_file),Toast.LENGTH_LONG).show();
